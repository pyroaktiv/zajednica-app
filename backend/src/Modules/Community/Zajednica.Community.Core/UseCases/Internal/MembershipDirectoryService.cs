...
public sealed class MembershipDirectoryService(IMembershipRepository memberships)
    : IInternalMembershipDirectoryService
{
    public IReadOnlyList<MemberAccountDto> GetAccounts(IReadOnlyCollection<Guid> membershipIds)
    {
        if (membershipIds.Count == 0)
            return [];

        return memberships.GetManyByIds(membershipIds)
