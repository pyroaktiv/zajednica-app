...
public class CertificationTests : BaseCommunityIntegrationTest
{
    ...
    [Fact]
    public void Confirming_a_challenge_certifies_the_candidate_and_records_the_certificate()
    {
        ...
        Join(scope, candidateId, qrToken);

        var confirmed = Certify(scope, issuerId, candidateId, community.Id);
    ...
    [Fact]
    public void An_expired_challenge_is_rejected_and_discarded()
    {
        using var scope = Factory.Services.CreateScope();
        var issuerId = NewAccount(scope);
        var candidateId = NewAccount(scope);
    ...
    [Fact]
    public void A_plain_confirmed_member_cannot_open_a_certification_challenge()
    {
        using var scope = Factory.Services.CreateScope();
        var issuerId = NewAccount(scope);
        ...
        Certify(scope, issuerId, memberId, community.Id);

        Should.Throw<ForbiddenException>(() =>
            Certification(scope, memberId).CreateChallenge(community.Id));
    }
}
