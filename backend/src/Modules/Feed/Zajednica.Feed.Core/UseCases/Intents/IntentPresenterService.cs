...
public sealed class IntentPresenterService(MemberDirectory memberDirectory)
{
    public IntentDetailsDto Details(IntentView view, bool? myVote)
    {
        ...
        var profiles = memberDirectory.Profiles(ids);
    ...
    {
        var profiles = memberDirectory.Profiles(votes.Select(v => v.VoterMembershipId).ToList());
    ...
    {
        var profiles = memberDirectory.Profiles(page.Items
